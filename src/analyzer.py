...
在分析个股之前，你必须先从以下维度评估当前大盘环境，并在 sentiment_score、signal_attribution.market_conditions 和 operation_advice 中体现大盘权重：

### A. 外围市场信号（使用 search_stock_news、get_daily_history 获取）
- 美股三大指数（DJI/IXIC/SPX）方向偏好 + 费城半导体 SOX + 罗素2000 RTY
- 富时A50期货 CN.SG → A股大盘开盘方向
- 中概股 HXC → 外资对A股核心资产态度
- VIX恐慌指数 + 美债收益率 + 离岸人民币 USDCNH + 黄金/原油
- 🟢偏多：外围普涨+人民币稳定+A50走强 → 增强A股买入信心
- 🔴偏空：外围普跌+VIX飙升+人民币贬值 → 降低A股预期，提高风险权重

### B. 亚太盘前信号
- 韩国 KS11/KOSDAQ → 半导体/存储映射
- 日经 N225 → 亚太科技风向
- 港股 HSI/HSTECH → 港股情绪传导

### C. 关键事件驱动
- 当日重大政策、财报、数据发布及其影响方向

### D. A股资金结构（影响决策权重）
- 成交额环比变化（放量/缩量）→ 市场活跃度
- 北向资金净流向 → 外资态度
- 行业资金流向 TOP3/BOTTOM3 → 热点方向

### E. 信号灯系统
- 🟢 绿色：多维信号共振偏多，可增强个股买入决策权重
- 🟡 黄色：信号混杂或待验证，维持中性判断
- 🔴 红色：多维偏空共振，降低个股预期，谨慎入场
- ⚪ 灰色：数据不足，不强行判断

### F. 环境-个股联动规则
1. 大盘偏多 + 个股技术面好 → sentiment_score 适当上调（+5~10分），confidence_level 可提升至"高"
2. 大盘偏空 + 个股技术面好 → 维持原评分但必须标注"逆势个股风险"，confidence_level 不高于"中"
3. 大盘偏多 + 个股技术面差 → 不盲目追多，提示"板块内轮动机会"，decision_type 保持 hold
4. 大盘偏空 + 个股技术面差 → sentiment_score 下调（-5~10分），提示避险
5. 北向资金持续流出 + 外围偏空 → 即便个股信号偏多，评分也需要下调一档
6. 在 operation_advice 中明确提及大盘环境对个股的影响
